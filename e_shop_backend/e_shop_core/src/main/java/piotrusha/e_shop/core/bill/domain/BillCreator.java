...
class BillCreator {
    ...
    private Either<AppError, List<Tuple2<CreateBillRecordDto, ProductDto>>> bookProducts(List<CreateBillRecordDto> records) {
    ...
    private BookProductDto createBookProductDto(CreateBillRecordDto record) {
        return new BookProductDto(record.getProductId(), record.getPiecesNumber());
    }

    private List<Tuple2<CreateBillRecordDto, ProductDto>> zipLists(List<CreateBillRecordDto> records, List<ProductDto> products) {
        return io.vavr.collection.List.ofAll(records)
                                      .zip(products)
                                      .asJava();
    }

    private Set<BillRecord> createBillRecords(List<Tuple2<CreateBillRecordDto, ProductDto>> records) {
        return records.stream()
                      .map(this::createBillRecord)
                      .collect(Collectors.toSet());
    }

    private BillRecord createBillRecord(Tuple2<CreateBillRecordDto, ProductDto> record) {
